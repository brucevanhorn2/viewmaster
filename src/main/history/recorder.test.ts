...
describe('recorder capture', () => {
  ...
  it('skips image files without reading or capturing them', async () => {
    await setup()
    const spy = vi.spyOn(content, 'readCurrentFile')
    const rec = createRecorder(repo.root, { historyBaseDir: base, settleMs: 5 })
    await repo.write('photo.png', Buffer.alloc(3 * 1024 * 1024, 0x61))
    rec.handleEvent('photo.png')
    await rec.flush()
    expect(spy).not.toHaveBeenCalled()
    expect(await readVersions(historyPaths(base, repo.root).logFile('photo.png'))).toEqual([])
    await rec.close()
    spy.mockRestore()
  })
