...
3. **State lives in two places, not a database.** GitHub issue labels are
   ...
   `factory:brainstorming`, `factory:executing`, `factory:awaiting-push`,
   `factory:in-review`, `factory:ready-to-merge`, `factory:needs-attention`),
   created once via `gh label create`. A local `.claude/factory/queue.md`
   holds the ordered triage queue and the conflict/parallel-group notes —
   working state, not shipped history. `.claude/factory/` is added to
   `.git/info/exclude`, the same mechanism already used for
   `.claude/worktrees/`.
4. **Only "executing" is capped; brainstorming is not.** The concurrency cap
   ...
   immediately, but the execution agent itself queues (FIFO) until a slot
   frees. This keeps the human never blocked, which is the stated goal.
...
`factory:queued`, `factory:brainstorming`, `factory:executing`,
`factory:awaiting-push`, `factory:in-review`, `factory:ready-to-merge`,
`factory:needs-attention`. An issue carries exactly one `factory:*` label at
a time (the dispatcher swaps it, not adds to it). No label = not yet
triaged.

### `.claude/factory/queue.md`
Human-readable ledger: ordered list of untriaged/queued issues with their
computed `{bug_or_feature, difficulty, likely_files}`, the conflict groups
derived from file overlap, and which issues currently occupy the 3 (or
`--cap N`) executing slots. Regenerated by `/factory triage`; updated
in-place by `/factory next` and the background execution agent as issues
change state.
...
### `/factory next`
Pops the next queue entry that doesn't conflict with anything currently
`factory:executing`. Relabels `factory:brainstorming`. Runs
`superpowers:brainstorming` interactively. On spec approval, proceeds
without further prompting: `writing-plans` → `git worktree add` → background
`Agent` running the plan. Relabels `factory:executing` (or, if all slots are
full, leaves the plan written and queues the execution agent, noting this in
`queue.md`). Returns control immediately either way.
...
relabels `factory:awaiting-push`, writes the exact `git push` command into
`queue.md` and as an issue comment, and frees its slot so the next queued
execution agent (if any) can start. On getting stuck, relabels
`factory:needs-attention` instead of committing incomplete/broken work.

### `/factory push-done <issue>`
Run by the human after pushing. Relabels `factory:in-review`, opens the PR
...
queued
  ...
  → executing (background agent) [may queue here if slots are full]
